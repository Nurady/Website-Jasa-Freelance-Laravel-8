        ...
        <nav class="mx-8 mt-8 text-sm lg:mx-20" aria-label="Breadcrumb">
            <ol class="inline-flex p-0 list-none">
                <li class="flex items-center">
                    <a href="#" class="text-gray-400">Service</a>
                    ...
                    <div class="flex overflow-x-scroll hide-scroll-bar dragscroll">
                        <div class="flex mt-2 flex-nowrap">
                            @forelse ($thumbnail as $item)
                                <img :class="{ 'border-4 border-serv-button': active === {{ $item->id }} }" @click="changeThumbnail('{{ url(Storage::url($item->thumbnail)) }}', {{ $item->id }})" src="{{ url(Storage::url($item->thumbnail)) }}" alt="thumbnail service" class="inline-block object-cover h-20 mr-2 rounded-lg cursor-pointer w-36">
                            @empty
                                {{-- empty --}}
                            @endforelse
                        </div>
                ...
                <div class="content">
                    <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div x-show.transition.duration.500ms="tab === 'description'" class="leading-8 text-md">
                            ...
                            <h3 class="my-4 text-lg font-semibold">Note</h3>
                            <p class="mb-4">
                                {{ $service->note ?? '' }}
                            </p>
                                ...
                                <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                                    <div class="flex items-center space-x-4">
                                        @if ($service->user->detail_user->photo != NULL)
                                            <img class="object-cover w-20 h-20 rounded-full" src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="photo profile" loading="lazy">
                                        @else
                                            <svg class="object-cover w-20 h-20 text-gray-300 rounded-full" fill="currentColor" viewBox="0 0 24 24">
            ...
            <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
                <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                    ...
                    <div class="flex items-start px-4 pt-6">
                        ...
                        @else
                            <svg class="object-cover w-16 h-16 mr-4 text-gray-300 rounded-full" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                            </svg>
                        @endif
                        <div class="w-full">
    ...
    <script>
        function gallery() {
            return {
                ...
                featured: '{{ url(Storage::url($item->first()->thumbnail)) }}',
                active: 1,
