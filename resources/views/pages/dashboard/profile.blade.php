                                ...
                                <div class="px-4 py-5 sm:p-6">
                                    <div class="grid grid-cols-6 gap-6">
                                        <div class="col-span-6">
                                            ...
                                            <div class="flex items-center mt-1">
                                                @if (auth()->user()->detail_user()->first()->photo != NULL)
                                                    <img src="{{ url(Storage::url(auth()->user()->detail_user()->first()->photo)) }}" alt="profile" class="w-16 h-16 rounded-full">
                                                @else
                                                ...
                                                <a href="{{ route('member.profile.photo.delete') }}" type="button" class="px-3 py-2 ml-5 text-sm font-medium leading-4 text-red-700 bg-transparent rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500" onclick="return confirm('Are You Sure want to delete photo??')">
                                                    Delete
                                                </a>
                                            </div>
                                            @if ($errors->has('photo'))
                                        ...
                                        <div class="md:col-span-6 lg:col-span-3">
                                            ...
                                            <input placeholder="WYour Role" type="text" name="role" id="role" autocomplete="role" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" value="{{ $user->detail_user->role ?? '' }}" required>
                                            @if ($errors->has('role'))
                                        ...
                                        <div class="col-span-6">
                                            ...
                                            @forelse ($experience_user as $key=>$item)
                                                <input placeholder="Your experience" type="text" name="{{ 'experience['.$item->id.']' }}" id="{{ 'experience['.$item->id.']' }}" autocomplete="{{ 'experience['.$item->id.']' }}" class="block w-full py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" value={{ $item->experience }}>
