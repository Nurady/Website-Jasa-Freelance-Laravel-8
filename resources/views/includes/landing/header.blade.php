        ...
        <div class="flex flex-row flex-wrap items-center justify-between mx-auto ">
            ...
            <div class="flex-wrap items-center justify-center hidden w-full text-base lg:flex lg:items-center lg:w-auto lg:ml-auto lg:mr-auto" id="menu">
                <nav
                    ...
                    <a href="{{ route('index') }}" class="block {{ request()->is('/') ? 'font-medium nav-link active' : 'nav-link text-serv-text' }}">Home</a>
                    <a href="{{ route('landing.explorer') }}" class="block {{ request()->is('explorer') ? 'font-medium nav-link active' : 'nav-link text-serv-text' }}">Service</a>
                    <a href="#" class="block nav-link text-serv-text">Procedure</a>
                    <a href="#" class="block nav-link text-serv-text">Testimonial</a>
                    ...
                    @auth
                        ...
                        <a 
                            href="{{ route('logout') }}" class="block lg:hidden nav-link text-serv-text"
                    ...
                    <button @click="open = !open" 
                        class="flex flex-row items-center w-full px-4 py-2 mt-2 text-left bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:focus:text-white dark-mode:hover:text-white dark-mode:focus:bg-gray-600 dark-mode:hover:bg-gray-600 md:w-auto md:inline md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                            ...
                            @if(auth()->user()->detail_user()->first()->photo != null)
                                <img class="inline w-12 h-12 ml-3 rounded-full" src="{{ url(Storage::url(auth()->user()->detail_user()->first()->photo)) }}" alt="" loading="lazy" />
                            @else
                                <svg class="inline w-12 h-12 ml-3 text-gray-300 rounded-full" fill="currentColor" viewBox="0 0 24 24">
